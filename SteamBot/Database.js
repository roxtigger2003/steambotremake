...
function __db_err(err, where) {
	if (err) {
		console.log(Helper.get_date_formated() + " Lỗi khi cố gắng thao tác thông tin trong cơ sở dữ liệu, " + where);
		console.log(err);
		return;
    }
}
...
Database.prototype.create_account = function(info, callback) {
	...
	if (!account.email && !account.password) {
		callback(true,"Bạn phải nhập email và mật khẩu");
		return;
	}
	...
	this.find_account(account.email, function(result) {
		if (result.length > 0) {
			callback(true, "Tài khoản đã được đăng ký");
		} else {
			self.datastore['accounts'].insert(account, function(err, result) { 
				__db_err(err, __fili);
				callback(false, "Đăng ký thành công");
			});
		}
	});
};
...
Database.prototype.update_account = function(email, new_data, callback) {
	...
	if (!email) {
		callback(true, "Bạn phải nhập địa chỉ email của tài khoản để được cập nhật.");
		return;
	}
	...
	this.find_account(email, function(result) {
		if (result.length > 0) {
			...
			if (!account.password) {
				callback(true, "Bạn phải nhập mật khẩu");
				return;
			}
			...
			self.datastore['accounts'].update({type: "account", email: account.email}, { $set: { 
				...
				if (numReplaced > 0) {
					callback(false, "Tài khoản được cập nhật thành công.");
				} else {
					callback(true, "Không thể cập nhật dữ liệu tài khoản");
				}
			});
		} else {
			callback(true, "Không tìm thấy tài khoản");
		}
	});
};
